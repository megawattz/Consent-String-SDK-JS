...
local decodeConsentString = require('decode').decodeConsentString;
local test = require('test')
local utils = require('utils')
local Date = require('date')

test.describe('decode', function () 
  local aDate = Date.new('2018-07-15 PDT');
  ...
  test.it('decodes the consent data from another base64-encoded string', function ()
    ...
    local toCompareWith = {
      ...
      lastUpdated = Date.new("2018-05-24T12:47:40.700Z"),
      version = 1,
    ...
    test.assert("consentData == toCompareWith");
    test.assert("consentData2 == toCompareWith");
    end)
end)
